						...
						WHERE infoYear = '$year' AND infoClass = $class";
										...
										?>
											...
											foreach ($groupsBy as $value) {
												// Decode JSON, force array output, suppress errors
												$opt = json_decode($value->infoOptionals, true);

												// If decode failed or returned something other than array → force empty array
												if (!is_array($opt)) {
													$opt = [];
												}

												// Handle 4th subject with safe decoding
												$decodedFrth = [];
												if (!empty($value->info4thSub)) {
													$tmp = json_decode($value->info4thSub, true);
													...
													if (!empty($decodedFrth)) {
														$opt = array_merge($opt, $decodedFrth);
													}
												}

												// Convert all optional subject IDs to integers for consistent comparison
												$opt = array_map('intval', $opt);
											?>
												<div id="itemMainBox">
													<div style="padding:0 15px 10px 15px; ">
														...
														<div style="float: left; clear: both;width: 100%;margin-bottom: 10px;">
															...
															<table style="width: 100%; border-collapse: collapse;margin-top: 15px;">
																...
																// Show only optional subjects that belong to this student
																if (is_array($opt) && sizeof($opt) > 0) {
																	foreach ($opt as $optSubjectId) {
																		if (isset($alloptSub[$optSubjectId])) {
																			$optSubjectDate = isset($subjectDates[$optSubjectId]) ? $subjectDates[$optSubjectId] : '';
																			rows($alloptSub[$optSubjectId]['name'], $optSubjectDate, $alloptSub[$optSubjectId]['code']);
																		}
																	}
																}
															...
															<p style="margin-top: 20px">N.B- Examinee must sign by ensuring date and subject. By ensuring examinees sign invigilator will put his/her sign.</p>
