...
// Extract teacher restrictions for use in queries when available
$teacherOfClass = '';
$teacherOfSection = '';
					...
					<form class="form-inline" action="" method="GET">
						...
						<div class="form-group">
							...
							<select id='resultClass' class="form-control" name="class" required>
								<?php
									if ($hasAssignedClass && $teacherOfClass !== '') {
										$classQuery = $wpdb->get_results(
											$wpdb->prepare(
												"SELECT classid,className FROM ct_class WHERE classid IN (SELECT examClass FROM ct_exam GROUP BY examClass ORDER BY className ASC) AND classid IN ($teacherOfClass)",
												null
											)
										);
									} else {
