...
<?php } ?>
	...
	<?php
		if(isset($_GET['exam'])):
			...
			if($exam != ''){
				$qry = "SELECT spStdID,stdName,infoRoll,withheld,spPoint,spTotalMark,spSubjTotal,sectionName,spFaild,spPosition,spClassPosition,spAbsent,infoOptionals,info4thSub FROM ct_studentPoint
					...
					WHERE spYear = '$year' AND spClass = $class AND spExam = $exam AND spFaild >= 3";

				if ($sec != '') { $qry .= " AND infoSection = $sec"; }

				if ($grou != '') { $qry .= " AND infoGroup = $grou"; }

				// $qry .= " ORDER BY spAbsent ASC, spFaild ASC, spPoint DESC, spTotalMark DESC, infoRoll ASC";
					  ...
					  <div class="printArea" style="margin: 10px 30px 0;">
				  		...
				  		<?php if($exam != ''){ ?>
					  		<table style="width: 100%; text-align: center;">
					  			<tr style="background: #FFE599;-webkit-print-color-adjust: exact;line-height: 1">
											...
											?>
												<tr style="line-height: 1">
								  				...
								  				<td><?= $student->spPoint ?></td>
								 					...
								 					<td><?= $student->withheld == 1?'Withheld': (($sec != '') ? $student->spClassPosition : $student->spPosition) ?></td>
					  							<td><?= $student->withheld == 1?'Withheld': (($sec != '') ? $student->spPosition : $student->spClassPosition) ?></td>
