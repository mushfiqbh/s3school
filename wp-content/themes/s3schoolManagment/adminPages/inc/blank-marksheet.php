...
if(isset($_GET['exam'])):
	...
	$info = $wpdb->get_results( "SELECT examName,className,subjectName,sectionName,connecttedPaper,subPaper,subOptinal,sub4th,subMCQ,subCQ,subPect FROM ct_subject
		LEFT JOIN ct_exam ON examid = $exam
		LEFT JOIN ct_section ON ct_section.sectionid = $sec
							...
							<div class="table-responsive">
								<table id="resultInputTable" class="table table-bordered" style="width: 100%">
									<tr>
										...
										<th>Name</th>
										<th style="text-align: center;"><?= $s3sRedux['cqtitle'] ?> (<?= $subCQ ?>)</th>
										<th style="text-align: center;"><?= $s3sRedux['mcqtitle'] ?> (<?= $subMCQ ?>)</th>
										<th style="text-align: center;"><?= $s3sRedux['prctitle'] ?> (<?= $subPect ?>)</th>
									</tr>
									...
									<?php
										...
										foreach ($stdQuery as $student) { ?>
											...
											<tr>
												...
												<td style="text-align: center;"><?= ($subCQ == 0) ? 'x' : ''; ?></td>
												<td style="text-align: center;"><?= ($subMCQ == 0) ? 'x' : ''; ?></td>
												<td style="text-align: center;"><?= ($subPect == 0) ? 'x' : ''; ?></td>
											</tr>
											<?php
										}
									?>
