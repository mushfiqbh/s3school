...
if(isset($_GET['exam'])):
	...
	$info = $wpdb->get_results( "SELECT examName,className,subjectName,combineMark,connecttedPaper,subPaper,subOptinal,sub4th,assessment,subMCQ,subCQ,subPect,subCa FROM ct_subject
								...
								<div class="table-responsive">
									<table id="resultInputTable" class="table table-bordered ">
										<tr>
											...
											<th>Sec</th>
											<th>Sub Type</th>
											<th><?= $assessment == 1? 'Hand Writing' : $s3sRedux['cqtitle'] ?> (<?= $subCQ ?>)</th>
											<th><?= $assessment == 1? 'Attendence' : $s3sRedux['mcqtitle'] ?> (<?= $subMCQ ?>)</th>
											<th><?= $assessment == 1? 'Neat & Clean' : $s3sRedux['prctitle'] ?> (<?= $subPect ?>)</th>
											<th><?= $assessment == 1? 'Uniform' : $s3sRedux['catitle'] ?> (<?= $subCa ?>)</th>
										</tr>
										...
										<?php
											...
											foreach ($stdQuery as $student) {
												...
												<input type="hidden" name="assessment[<?= $student->studentid ?>]" value='<?= $assessment ?>'>
