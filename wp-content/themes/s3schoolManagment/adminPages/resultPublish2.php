...
if (isset($_POST['unpublisRes'])) {
	...
	$update = $wpdb->query( $wpdb->prepare( "UPDATE ct_result SET status = %d WHERE resultYear = %d AND resClass = %d AND resExam = %d", 0,$year, $class , $exam	) );
														  				...
														  				<tr>
														  					...
														  					<td>
														  						<form action="" method="POST">
														  							...
														  							<button class="btn btn-success btn-sm" type="submit" name="publisRes">Publish</button>
														  						</form>
														  					</td>
														  				</tr>
															  		<?php
									  		...
									  		?>
													...
													<div class="bs-example">
													  <table class="table table-striped table-bordered">
													    ...
													    <tbody>
													      <?php 
															  	$exams = $wpdb->get_results( "SELECT resExam,examName FROM `ct_result` LEFT JOIN ct_exam ON ct_result.resExam = ct_exam.examid WHERE resultYear = '$resYear' AND resClass = $resClass AND status = 1 GROUP BY resExam ORDER BY resExam ASC" );
															  	foreach ($exams as $exam) {
															  		$resExam = $exam->resExam;
															  		?>
														  				...
														  				<tr>
														  					...
														  					<td>
														  						<form action="" method="POST">
														  							...
														  							<input type="hidden" name="resClass" value="<?= $resClass ?>">
														  							<input type="hidden" name="resExam" value="<?= $resExam ?>">
														  							<button class="btn btn-success btn-sm" type="submit" name="unpublisRes">Unpublish</button>
														  						</form>
														  					</td>
														  				</tr>
															  		<?php
