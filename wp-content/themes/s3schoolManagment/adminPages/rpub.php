	...
	foreach ($users as $userkey => $user) {
		...
		foreach ($combines as $combin) {
			...
			if($combin->subPaper == 1){
				...
				$allsubjTotal += $subTot1*$convertPercent/100 + $combin->subCa;
				
				$obtain = ((isnum($combin->resCQ)+isnum($combin->resMCQ)+isnum($combin->resPrec))*$convertPercent/100)+isnum($combin->resCa);
				$subTot2 = 0;
				...
				foreach ($combines as $combin2) {
					if($combin2->connecttedPaper == $combin->resSubject){
						...
						$havecon = true;
						$obtain += ((isnum($combin2->resCQ)+isnum($combin2->resMCQ)+isnum($combin2->resPrec))*$convertPercent/100)+isnum($combin2->resCa);

						$subTot2 = (isnum($combin2->subMCQ)+isnum($combin2->subCQ)+isnum($combin2->subPect))*$convertPercent/100 + $combin2->subCa;
						
						$allsubjTotal += $subTot2*$convertPercent/100 + $combin2->subCa;
						...
						$reCa2 = $resPre = isnum($combin2->resCa);
...
// 			$absentCk[] = $result2->resCa;
			...
			$resTotal = ((isnum($result2->resCQ)+isnum($result2->resMCQ)+isnum($result2->resPrec))*$convertPercent/100)+isnum($result2->resCa);
			...
			$allsubjTotal += $subjTotal*$convertPercent/100 + $result2->subCa;
