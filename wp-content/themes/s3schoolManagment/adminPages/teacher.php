...
<div class="container-fluid maxAdminpages" style="padding-left: 0">
	...
	<style>
		.switch { position: relative; display: inline-block; width: 44px; height: 22px; margin-bottom: 0; vertical-align: middle; }
	...
	<?php $teacher_restrictions_enabled = $wpdb->get_var("SELECT option_value FROM sm_options WHERE option_name = 's3s_teacher_restrictions_enabled'"); ?>

	<h2>
		Teacher Management 
		<div class="pull-right" style="display: flex; align-items: center; gap: 15px;">
			<div style="display: flex; align-items: center;" title="Teacher can only access their assigned class/section if enabled.">
				<label class="switch">
					<input type="checkbox" id="toggleRestrictions" <?= $teacher_restrictions_enabled ? 'checked' : '' ?>>
					<span class="slider"></span>
				</label>
				<span style="font-size: 14px; font-weight: normal; margin-left: 5px;">Access<br>Restrictions</span>
			</div>
			<a href="?page=teacher" class="btn btn-success">Add Teacher</a> 
		</div>
	</h2><br>
	<div class="row">
	...
	(function($) {
		$(document).ready(function() {
	      ...
	      $('#teacherOfSection').prop('disabled', false);
			...
			$('#toggleRestrictions').change(function() {
				var isEnabled = $(this).is(':checked') ? 1 : 0;
				$.ajax({
					url: $siteUrl + "/inc/ajaxAction.php",
					method: "POST",
					data: { 
						type: 'toggleTeacherRestrictions', 
						enabled: isEnabled 
					},
					success: function(response) {
						if(response === 'success') {
							// Optional: Show a quick toast or message
						}
					}
				});
			});
