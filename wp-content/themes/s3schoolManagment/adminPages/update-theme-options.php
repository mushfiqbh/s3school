...
<script>
    jQuery(document).ready(function($) {
        $('.media_upload_button').on('click', function(e) {
            ...
            var modalTitle = button.data('title') || 'Select or Upload a File';
            var modalButtonText = button.data('button-text') || 'Use this file';
            var allowedTypesRaw = button.data('allowed-types') || '';
            var allowedTypes = allowedTypesRaw
                .toString()
                .split(',')
                .map(function(type) {
                    return type.trim();
            ...
            mediaFrame.on('select', function() {
                ...
                var previewSource = fullUrl;
                if (attachment.sizes && attachment.sizes.thumbnail && attachment.sizes.thumbnail.url) {
                    previewSource = attachment.sizes.thumbnail.url;
                    fieldset.find('.upload-thumbnail').val(attachment.sizes.thumbnail.url);
                } else if (attachment.icon) {
                    previewSource = attachment.icon;
                    fieldset.find('.upload-thumbnail').val(attachment.icon);
                } else {
                    fieldset.find('.upload-thumbnail').val(fullUrl);
                }

                var previewImage = fieldset.find('.redux-option-image');
                if (previewImage.length) {
                    previewImage.attr('src', previewSource);
                    if (previewSource) {
                        previewImage.show();
                    }
                }

                var previewLink = fieldset.find('.media-file-link');
                if (previewLink.length) {
                    previewLink.attr('href', fullUrl);
                    var fileLabel = attachment.filename || attachment.title || fullUrl.split('/').pop();
                    previewLink.text(fileLabel);
                    fieldset.find('.media-file-meta').show();
                }
            });
        ...
        $('.remove-image').on('click', function(e) {
            ...
            var previewLink = fieldset.find('.media-file-link');
            if (previewLink.length) {
                previewLink.attr('href', '#').text('');
            }
            fieldset.find('.media-file-meta').hide();
        });
