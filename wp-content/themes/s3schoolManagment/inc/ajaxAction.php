...
// 		$subjects = $wpdb->get_results( "SELECT subjectid,subjectName FROM ct_subject WHERE subjectClass = '$class' AND subOptinal = 1 AND (forGroup IN ('$group', 'all') OR forGroup LIKE '%\"$group\"%') ORDER BY subjectName" );
	...
	*/
	...
	if ($applyTeacherRestrictions && $current_user->roles[0] == 'um_teachers') {
        $current_user_id = get_current_user_id();
        $teacher_record = $wpdb->get_row($wpdb->prepare("SELECT tecAssignSub, teacherOfClass FROM ct_teacher WHERE tecUserId = %d", $current_user_id));

        if ($teacher_record) {
            // Check if teacher is class teacher for the exam's class
            $exam_class = $wpdb->get_var($wpdb->prepare("SELECT examClass FROM ct_exam WHERE examid = %d", $exam));
            $is_class_teacher = (!empty($teacher_record->teacherOfClass) && $teacher_record->teacherOfClass == $exam_class);
            
            // Only filter by assigned subjects if NOT a class teacher
            if (!$is_class_teacher && !empty($teacher_record->tecAssignSub)) {
                $assigned_subjects = json_decode($teacher_record->tecAssignSub, true);
            ...
            // If is_class_teacher is true, don't filter - show all subjects
        }
    }
