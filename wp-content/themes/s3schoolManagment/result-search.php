	...
	<div class="b-layer-main">
		...
		<div class="b-blog-classic">
				...
				<div class="row">
					<div class="col-md-12">
						...
						if(!isset($_GET['roll'])){
							...
							$haveRes = $wpdb->get_results("SELECT * FROM `ct_result` WHERE resClass = $class AND resExam = $exam AND resultYear = '$year' AND resStdRoll = $stdRoll AND status = 1 and can_student_search_result = 1");
							if(sizeof($haveRes) > 0){
            ...
            $total_subCa = $ids ? $wpdb->get_var("SELECT IFNULL(SUM(subCa), 0) FROM ct_subject WHERE subjectid IN ($ids)") : 0;
				  			...
				  			$quey .= $stdRoll != '' ? "  AND infoRoll = '$stdRoll'" : '';
																...
																<tbody style="position: relative; line-height: 1; font-size: 13px">
																	<?php
																		...
																		$results2 = $wpdb->get_results( "SELECT * FROM `ct_result`
																			LEFT JOIN ct_subject ON ct_result.resSubject = ct_subject.subjectid
																			LEFT JOIN ct_class ON $class = ct_class.classid
																			WHERE resClass = $class AND resExam = $exam AND resultYear = '$year' AND resStdRoll = '$roll' AND resStudentId = $stdnt AND subCombineMark = 0 GROUP BY resSubject ORDER BY resSub4th,subOptinal,subCode ASC" );
